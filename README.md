...
2 เปิดเบราว์เซอร์ไปที่ลิงก์นี้เพื่อสร้างสคริปต์ใหม่: script.google.com
...
10 ระบบจะบังคับให้เราให้สิทธิ์ (Authorize access):
11 กดปุ่ม ให้สิทธิ์เข้าถึง (Authorize access)

/หากเจอหน้าต่างคำเตือนความปลอดภัย (Google hasn't verified this app) ให้คลิก "ขั้นสูง" (Advanced) (จะอยู่ด้านล่างๆ เล็กๆ)
แล้วคลิก "ไปที่ โปรเจกต์ที่ไม่มีชื่อ (ไม่ปลอดภัย)" (Go to Untitled project (unsafe))
กด อนุญาต (Allow)/

เมื่อเสร็จแล้ว คุณจะได้ "URL ของเว็บแอป" (Web app URL) (ที่ลงท้ายด้วย /exec) ให้คัดลอกลิงก์นั้นไว้ครับ
```javascript
function doPost(e) {
  try {
