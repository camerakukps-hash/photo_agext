...
async function fetchCollections() {
    const cachedData = localStorage.getItem('adminGalleryCache');
    if (cachedData) {
        try {
            ...
            if (settingsRecord && settingsRecord.urls) {
                GAS_URLS = settingsRecord.urls.map(u => typeof u === 'string' ? { url: u, active: true, note: "" } : u);
            } else {
                GAS_URLS = [{ url: "https://script.google.com/macros/s/AKfycbxXJYlJ-ycy0nXKV4JjAorFRVbvF2tJEDxn8xwIOItduv0MKpcOVnrkB79UgpKX2yBt/exec", active: true, note: "ไดรฟ์ดั้งเดิม" }];
            }
    ...
    try {
        ...
        if (isDirty) return; // ป้องกันการเซฟทับถ้าผู้ใช้พิมพ์ข้อมูลไปแล้วระหว่างโหลด
        
        const allRecords = Array.isArray(data.record) ? data.record : (Array.isArray(data) ? data : []);
        
        localStorage.setItem('adminGalleryCache', JSON.stringify(allRecords));
...
async function saveCollections(galleriesData) {
    ...
    try {
        ...
        localStorage.setItem('adminGalleryCache', JSON.stringify(payload));
...
function renderAdminList(collections) {
    ...
    sorted.forEach(item => {
        ...
        const hideTitle = item.hidden ? 'เลิกซ่อน (แสดงในหน้าหลัก)' : 'ซ่อนโฟลเดอร์นี้จากคนทั่วไป';
...
window.editCollection = async function(id) {
    ...
    const newName = prompt('1/3 แก้ไขชื่ออัลบั้ม:', item.name);
